window.addEventListener("load", ()=>{
    ...
    fetch('/admin/country', {method:"GET", headers: {"Authorization" : `Bearer ${token}`}})
...
function createCart(){
    ...
    $("#countryModal").modal('show');
        ...
        fetch("/admin/country/addCountry", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(e){
            e.preventDefault();
            fetch("/admin/country/deleteCountry/" + id, {method:"DELETE", headers:{'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.error){
                    alert(data.error);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editCountryBtn").addEventListener("click", function(e){
                ...
                fetch("/admin/country/edit/"+country_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
