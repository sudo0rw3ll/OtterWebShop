window.addEventListener("load", ()=>{
    ...
    fetch('/admin/productCategory', {method:"GET", headers:{'Authorization' : `Bearer ${token}`}})
...
function createCart(){
    ...
    $("#productCatModal").modal('show');
        ...
        fetch("/admin/productCategory/addProductCategory", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(){
            fetch("/admin/productCategory/deleteProductCategory/" + id, {method:"DELETE", headers:{'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.msg){
                    alert(data.msg);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(){
            ...
            document.getElementById("editProductCatBtn").addEventListener("click", function(){
                ...
                fetch("/admin/productCategory/editProductCategory/"+product_cat_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
