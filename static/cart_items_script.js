window.addEventListener("load", ()=>{
    ...
    fetch('/admin/cartItem', {method:"GET", headers:{'Authorization' : `Bearer ${token}`}})
...
function createCartItem(){
    ...
    $("#cartItemModal").modal('show');
        ...
        fetch("/admin/cartItem/addCartItem", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(e){
            e.preventDefault();
            fetch("/admin/cartItem/deleteCartItem/" + id, {method:"DELETE", headers: {'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.error){
                    alert(data.error);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editCartItemBtn").addEventListener("click", function(e){
                ...
                fetch("/admin/cartItem/editCartItem/"+cart_item_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
