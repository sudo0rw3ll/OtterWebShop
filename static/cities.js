window.addEventListener("load", ()=>{
    ...
    fetch('/admin/city', {method:"GET", headers:{"Authorization" : `Bearer ${token}`}})
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(e){
            e.preventDefault();
            fetch("/admin/city/deleteCity/" + id, {method:"DELETE", headers:{"Authorization" : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.msg){
                    alert(data.msg);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editCityBtn").addEventListener("click", function(e){
                ...
                fetch("/admin/city/editCity/"+city_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
                ...
                .then(data => {
                    if(data.msg){
                        ...
                        fetch("/admin/city", {method:"GET", headers:{'Authorization' : `Bear ${token}`}})
                        .then(response => response.json())
