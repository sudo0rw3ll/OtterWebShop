window.addEventListener("load", ()=>{
    ...
    fetch('/admin/comment', {method:"GET", headers: {"Authorization" : `Bearer ${token}`}})
...
function createCart(){
    ...
    $("#commentModal").modal('show');
        ...
        fetch("/admin/comment/addComment", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(e){
            e.preventDefault();
            fetch("/admin/comment/deleteComment/" + id, {method:"DELETE", headers:{'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.error){
                    alert(data.error);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editCommentBtn").addEventListener("click", function(e){
                ...
                fetch("/admin/comment/editComment/"+comment_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
