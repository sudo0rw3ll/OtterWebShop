window.addEventListener("load", ()=>{
    ...
    fetch('/admin/orderItem', {method:"GET", headers:{'Authorization' : `Bearer ${token}`}})
...
function createCart(){
    ...
    $("#orderItemModal").modal('show');
        ...
        fetch("/admin/orderItem/addOrderItem", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(e){
            e.preventDefault();
            fetch("/admin/orderItem/deleteOrderItem/" + id, {method:"DELETE", headers:{'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.msg){
                    alert(data.msg);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editOrderItemBtn").addEventListener("click", function(e){
                ...
                fetch("/admin/orderItem/editOrderItem/" + order_item_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
