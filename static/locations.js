window.addEventListener("load", ()=>{
    ...
    fetch('/admin/location', {method:"GET", headers: {'Authorization' : `Bearer ${token}`}})
...
function createCart(){
    ...
    $("#locationsModal").modal('show');
        ...
        fetch("/admin/location/addLocation", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(e){
            e.preventDefault();
            fetch("/admin/location/deleteLocation/" + id, {method:"DELETE", headers: {'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                if(data.error){
                    alert(data.error);
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editAddressBtn").addEventListener("click", function(e){
                ...
                fetch("/admin/location/editLocation/"+location_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
