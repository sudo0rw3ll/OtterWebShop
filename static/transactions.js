window.addEventListener("load", ()=>{
    ...
    fetch('/admin/transaction', {method:"GET", headers:{'Authorization' : `Bearer ${token}`}})
...
function createCart(){
    ...
    $("#transactionModal").modal('show');
        ...
        fetch("/admin/transaction/addTransaction", {
            method:"POST",
            headers:{
...
function updateTable(data){
    ...
    for(i=0;i<obrisi_buttons.length;i++){
        ...
        obrisi_buttons[i].addEventListener("click", function(){
            fetch("/admin/transaction/deleteTransaction/" + id, {method:"DELETE", headers: {'Authorization' : `Bearer ${token}`}})
            .then(response => response.json())
            .then(data => {
                fetch("/admin/transaction", {method:"GET", headers:{'Authorization' : `Bearer ${token}`}})
                .then(response => response.json())
    ...
    for(i=0;i<izmeni_buttons.length;i++){
        izmeni_buttons[i].addEventListener("click", function(e){
            ...
            document.getElementById("editTransactionBtn").addEventListener("click", function(){
                ...
                fetch("/admin/transaction/editTransaction/"+transaction_id, {
                    method: "PUT",
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type' : 'application/json',
